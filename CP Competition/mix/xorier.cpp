...
int main()
{
    ...
    while(t--)
    {
		...
		ep=(ull)((ull)(n*(n-1))/2)-(ull)(even*odd);
		sort(all(a1));
		sort(all(a2));
		for(unsigned int i=1;i<a1.size();i++)
		{
			if(a1[i]-2==a1[i-1] && (a1[i-1]%4==0))
			{
				ep-=(ull)(count[a1[i]]*count[a1[i-1]]);
			} 
			else if(a1[i]==a1[i-1])
			{
				ep-=((ull)(count[a1[i]]*(count[a1[i]]-1))/2);
			}
		}
		for(unsigned int i=1;i<a2.size();i++)
		{
			if(a2[i]-2==a2[i-1] && ((a2[i-1]-1)%4==0))
			{
				ep-=(ull)(count[a2[i]]*count[a2[i-1]]);
			}
			else if(a2[i]==a2[i-1])
			{
				ep-=((ull)(count[a2[i]]*(count[a2[i]]-1))/2);
			}
		}
		cout<<ep<<endl;
	}
