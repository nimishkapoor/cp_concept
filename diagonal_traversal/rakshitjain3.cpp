	#include<bits/stdc++.h>
	using namespace std;
	typedef long long int ll;

	#define watch(x) cout << (#x) << " is " << (x) << endl
	#define mod(x, m) ((((x) % (m)) + (m)) % (m))
	#define max3(a, b, c) max(a, max(b, c))
	#define min3(a, b, c) min(a, min(b, c))

	#define gcd __gcd
	#define nof1 __builtin_popcountll

	typedef long long ll;
	typedef unsigned long long ull;

	typedef pair<int, int> ii;
	typedef pair<int, ii> iii;
	typedef vector<int> vi;
	typedef vector<ii> vii;
	typedef vector<iii> viii;
	typedef vector<long long int> vl;
		
	const double pi = 2 * acos(0.0);
	const int inf = 0x3f3f3f3f;
	const double infd = 1.0/0.0;

	#define pb push_back
	#define mp make_pair
	#define fr first
	#define sc second
	#define clr(a) memset(a, 0, sizeof(a))
	#define all(v) v.begin(), v.end()
	#define alla(a,n) a, a+n
	#define endl "\n"

	long long power(long long x, long long y,long long MOD)
	{
		long long res = 1; 
		x = x % MOD; 

		while (y > 0)
		{
			if (y & 1)
			res = (res*x) % MOD;
			y = y>>1;
			x = (x*x) % MOD;
		}
		return res;
	}
	long long mul(long long a,long long b,long long MOD)
	{
	 if(b==1)
	 {
	  return a;
	 }	
	 if(b%2==0)
	 {
	  return 2*(mul(a,b/2,MOD)%MOD);
	 }
	 else
	 {
	  return (a+(2*(mul(a,b/2,MOD))%MOD))%MOD;
	 }
	}

	ll ncr[6001][6001];
	void nCr(long long MOD)
	{
		for(ll i=0;i<=6000;i++)
		{
			for(ll k=0;k<=6000;k++)
			{
				if(k<=i)
				{
					if((k==0)||(k==i))
					ncr[i][k]=1;
					else
					ncr[i][k]=(ncr[i-1][k-1]+ncr[i-1][k])%MOD;
				}
			}
		} 
	}

	ll inv[100005];
	void mulmodinv(long long MOD)
	{
		inv[1]=1;
		inv[0]=1;
		for(int i = 2; i <= 100005; i++)
		inv[i] = inv[MOD % i]*(MOD-MOD / i) % MOD;
	}

	bool ispow2(ll n)
	{
		if(((n & (n-1)) == 0) && n!=0)
		{
			return true;
		}
		return false;
	}

	int main()
	{
		ios_base::sync_with_stdio(0); 
		cin.tie(0);
		int n,m;
		string x;
		cin>>n>>m;
		vector<string> a;
		int dpr[n+1][m+1];
		int dpl[n+1][m+1];
		int dpu[n+1][m+1];
		int dpd[n+1][m+1];
		int up[n+1][m+1];
		int dw[n+1][m+1];
		clr(dpr);
		clr(dpl);
		clr(dpu);
		clr(dpd);
		clr(up);
		clr(dw);
		for(int i=0;i<n;i++)
		{
			cin>>x;
			a.pb(x);
		}
		for(int i=0;i<n;i++)
		{
			for(int j=0;j<m;j++)
			{
				if(i==0)
				{
					if(a[i][j]=='1')
					{
						up[i][j]=1;
					}
				}
				else if(a[i][j]=='1')
				{
					up[i][j]=1+up[i-1][j];
				}
			}
		}
		for(int i=n-1;i>=0;i--)
		{
			for(int j=m-1;j>=0;j--)
			{
				if(a[i][j]=='1')
				{
					dw[i][j]=1+dw[i+1][j];
				}
			}
		}
		
		for(int i=n-1;i>=0;i--)
		{
			for(int j=m-1;j>=0;j--)
			{
				if(a[i][j]=='1')
				{
					dpr[i][j]=1+dpr[i][j+1];
				}
			}
		}
		for(int i=0;i<n;i++)
		{
			for(int j=0;j<m;j++)
			{
				if(j==0)
				{
					if(a[i][j]=='1')
					{
						dpl[i][j]=1;
					}
				}
				else if(a[i][j]=='1')
				{
					dpl[i][j]=1+dpl[i][j-1];
				}
			}
		}	
		for(int i=0;i<n;i++)
		{
			for(int j=0;j<m;j++)
			{
				if(i==0 || j==0)
				{
					if(a[i][j]=='1')
					{
						dpu[i][j]=1;
					}
				}
				else if(a[i][j]=='1')
				{
					dpu[i][j]=1+dpu[i-1][j-1];
				}
			}
		}
		for(int i=n-1;i>=0;i--)
		{
			for(int j=m-1;j>=0;j--)
			{
				if(a[i][j]=='1')
				{
					dpd[i][j]=1+dpd[i+1][j+1];
				}
			}
		}	
		
		int ans=0;
		int r[n+5][m+5],l[n+5][m+5];
		clr(r);
		clr(l);
		for(int i=0;i<n;i++)
		{
			for(int j=0;j<m;j++)
			{
				r[i][j]=min3(dpr[i][j],dw[i][j],dpd[i][j]);
				l[i][j]=min3(dpl[i][j],up[i][j],dpu[i][j]);
			}
		}
		/*for(int i=0;i<n;i++)
		{
			for(int j=0;j<m;j++)
			{
				cout<<r[i][j]<<" ";
			}
			cout<<endl;
		}
		cout<<endl;
		for(int i=0;i<n;i++)
		{
			for(int j=0;j<m;j++)
			{
				cout<<l[i][j]<<" ";
			}
			cout<<endl;
		}
		cout<<endl;*/
		int k,p,q,i,j,s,t;
		for(k=0;k<n;k++)
		{
			p=k;
			q=0;
			s=p;
			t=q;
			for(i=1;i<=(max(n,m)-k);i++)
			{
				s=p;
				t=q;
				for(j=i;j<=(max(n,m)-k);j++)
				{//watch(r[p][q]);watch(l[s][t]);watch(i);watch(j);
					//watch(p);watch(q);
					//cout<<endl;//watch(s);watch(t);
					//if(j>=m)
					//{
					//	break;
					//}
					if(r[p][q]-1>=(j-i) && l[s][t]-1>=(j-i) && i<=j)
					{
						ans++;//watch(ans);
					}
					s++;
					t++;
					if(s>=n && t>=m)
					{
						break;
					} 
					
				}
				p++;
				q++;
				if(p>=n && q>=m)
				{
					break;
				}
				
			}
		}
		for(k=1;k<m;k++)
		{
			p=0;
			q=k;
			s=p;
			t=q;
			for(i=1;i<=(max(n,m)-k);i++)
			{
				s=p;
				t=q;
				for(j=i;j<=(max(n,m)-k);j++)
				{//watch(r[p][q]);watch(l[s][t]);watch(i);watch(j);
					//watch(s);watch(t);
					//watch(p);watch(q);
					//cout<<endl;
					if(r[p][q]-1>=(j-i) && l[s][t]-1>=(j-i) && i<=j)
					{
						ans++;//watch(ans);
					}
					s++;
					t++;
					if(s>=n && t>=m)
					{
						break;
					}
					//if(j>=m)
					//{
					//	break;
					//} 
				}
				p++;
				q++;
				if(p>=n && q>=m)
				{
					break;
				}
				
			}
		}
		cout<<ans<<endl;
		return 0;
	}
